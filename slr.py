...
class SLRParser:
    ...
    def print_info(self):
        ...
        for i, (head, body) in enumerate(self.G_indexed):
            aug.append(f'{i:>{len(str(len(self.G_indexed) - 1))}}: {head:>{self.max_G_prime_len}} -> {" ".join(body)}')
            print(f'{i:>{len(str(len(self.G_indexed) - 1))}}: {head:>{self.max_G_prime_len}} -> {" ".join(body)}')
        print(aug)

        # create list aug into dataframe and save it
        df = pd.DataFrame(aug)
        df.to_csv('augmented_grammar.csv', index=False, header=False)
        ...
        # save terminals, nonterminals and symbols into csv file
        df = pd.DataFrame(self.G_prime.terminals)
        df.to_csv('terminals.csv', index=False, header=False)

        df = pd.DataFrame(self.G_prime.nonterminals)
        df.to_csv('nonterminals.csv', index=False, header=False)

        df = pd.DataFrame(self.G_prime.symbols)
        df.to_csv('symbols.csv', index=False, header=False)

        first=[]
        print('\nFIRST:')
        for head in self.G_prime.grammar:
            first.append(f'{head:>{self.max_G_prime_len}}: {" ".join(self.first[head])}')
            print(f'{head:>{self.max_G_prime_len}} = {{ {", ".join(self.first[head])} }}')

        follow=[]
        print('\nFOLLOW:')
        for head in self.G_prime.grammar:
            follow.append(f'{head:>{self.max_G_prime_len}}: {" ".join(self.follow[head])}')
            print(f'{head:>{self.max_G_prime_len}} = {{ {", ".join(self.follow[head])} }}')

        # save first and follow into csv file
        df = pd.DataFrame(first)
        df.to_csv('first.csv', index=False, header=False)

        df = pd.DataFrame(follow)
        df.to_csv('follow.csv', index=False, header=False)
        ...
        # save parse table into csv file
        ...
        # save parse table into image png file

        # create list of list for parse table
        parse_table_list = []
        for r in range(len(self.C)):
            parse_table_list.append([])
            for c in self.parse_table_symbols:
                parse_table_list[r].append(self.parse_table[r][c])
        
        # create dataframe for parse table
        df = pd.DataFrame(parse_table_list, columns=self.parse_table_symbols)
        df.to_csv('parse_table.csv', index=False, header=True)
    ...
    def generate_automaton(self):
        ...
        automaton.render('automaton.gv', view=False)
    ...
    def LR_parser(self, w):
        ...
        # save parsing results into dataframe
        df = pd.DataFrame(results)
        df.to_csv('parsing_results.csv', index=False, header=True)
